...
The final model analysis shows that the model needs improvements before deployment for commercial use.
...
From @fig-cat-dist above, we can see that there is class imbalance in the target (diagnosis of heart disease). This means we will need to consider balanced scoring metrics and models.
...
Since this data set is somewhat imbalanced (~80/20 split on non-disease vs disease), 
...
The model created needs further work and is not ready for deployment. 

Our final test results yielded a F1 score of `{python} best_test_f1` and there is a large discrepancy to the training F1 score (`{python} best_train_f1`). This indicates that the model is overfitting. It is important to note that the test dataset is small with only 22 observations, which is likely contributing to the low F1 score.

Furthermore, our model has an accuracy of `{python} best_test_accuracy` percent. This value is worse than the baseline dummy accuracy, which again indicates that further work is required to improve the model.
...
However, the model performed poorly on F1 score and was not able to minimize the number of false negatives classified 
