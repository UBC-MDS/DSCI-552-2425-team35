...
lr_cv_f1 = cv_results_table.iloc[5, 3]
...
Applying it on our data set gave approximately `{python} best_test_accuracy` percent accuracy. 
...
The final F1 score on test data is found to be `{python} best_test_f1`.
...
returning a recall value of `{python} best_test_recall`.
...
being able to correctly classify presence of angiographic coronary disease with a decent level of accuracy (`{python} best_test_accuracy` percent). 
